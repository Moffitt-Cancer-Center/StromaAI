...
- **Operational scripts** — `scripts/` contains helpers for nginx deployment (`deploy-nginx.sh`), system status (`status.sh`), config validation (`check-config.sh`), API key rotation (`rotate-api-key.sh`), planned maintenance (`drain-and-restart.sh`), debug bundles (`debug-bundle.sh`), and Grafana dashboard generation (`generate-grafana-dashboard.sh`). See [Operational Scripts](docs/deployment-guide.md#operational-scripts) for usage and deployment workflow integration.
