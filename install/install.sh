...
_deploy_config_env() {
    ...
    # If config exists, offer to merge instead of overwrite
    if [[ -f ${STROMA_INSTALL_DIR}/config.env ]]; then
        backup_file "${STROMA_INSTALL_DIR}/config.env"
        
        if [[ "${STROMA_YES}" == "1" ]]; then
            log_info "Auto-merging existing config with new defaults (--yes mode)."
            _merge_config=1
        else
            echo -e "${YELLOW}Existing config.env found. Choose action:${RESET}"
            echo "  1) Merge — update with new defaults, preserve manual edits"
            echo "  2) Overwrite — replace entire file with generated config"
            echo "  3) Keep — leave existing file unchanged"
            read -r -p "Action [1-3]: " _action
            case "${_action}" in
                1) _merge_config=1 ;;
                2) _merge_config=0 ;;
                3) log_info "Keeping existing config.env."; return 0 ;;
                *) log_info "Invalid choice — keeping existing config.env."; return 0 ;;
            esac
        fi
    else
        _merge_config=0  # New file, no merge needed
    fi

    if [[ "${STROMA_DRY_RUN}" == "0" ]]; then
        # Determine target file: temp for merge, direct for overwrite
        local target_file
        if [[ "${_merge_config:-0}" == "1" ]]; then
            target_file="${STROMA_INSTALL_DIR}/config.env.new"
        else
            target_file="${STROMA_INSTALL_DIR}/config.env"
        fi
...
STROMA_SLURM_BIN_DIR=${STROMA_SLURM_BIN_DIR:-}
...
STROMA_SLURM_CPUS=${STROMA_SLURM_CPUS:-64}
STROMA_SLURM_MEM=${STROMA_SLURM_MEM:-900G}
STROMA_SLURM_GRES=${STROMA_SLURM_GRES:-}
...
# Gateway — OIDC authentication proxy
# Populated by setup-keycloak.sh / setup-head.sh after install.
GATEWAY_PORT=${GATEWAY_PORT:-9000}
GATEWAY_INTERNAL_URL=${GATEWAY_INTERNAL_URL:-http://127.0.0.1:${GATEWAY_PORT:-9000}}
GATEWAY_VERIFY_SSL=${GATEWAY_VERIFY_SSL:-true}
VLLM_BACKEND_URL=${VLLM_BACKEND_URL:-http://127.0.0.1:${STROMA_VLLM_PORT}}

# OIDC / Keycloak — populated by setup-keycloak.sh
OIDC_DISCOVERY_URL=${OIDC_DISCOVERY_URL:-}
OIDC_AUDIENCE=${OIDC_AUDIENCE:-stroma-gateway}
JWKS_OVERRIDE_URI=${JWKS_OVERRIDE_URI:-}
KC_GATEWAY_CLIENT_ID=${KC_GATEWAY_CLIENT_ID:-stroma-gateway}
KC_GATEWAY_CLIENT_SECRET=${KC_GATEWAY_CLIENT_SECRET:-}
...
EOF
        ...
        # If merge mode, extract and preserve custom variables
        if [[ "${_merge_config:-0}" == "1" ]]; then
            local managed_vars=("STROMA_INSTALL_DIR" "STROMA_SHARED_ROOT" "STROMA_HEAD_HOST" \
                "STROMA_VLLM_PORT" "VLLM_INTERNAL_URL" "STROMA_HTTPS_PORT" "STROMA_RAY_PORT" \
                "STROMA_RAY_DASHBOARD_PORT" "STROMA_API_KEY" "KC_INTERNAL_URL" "OPENWEBUI_INTERNAL_URL" \
                "STROMA_MODEL_PATH" "STROMA_MODEL_NAME" "STROMA_CONTAINER" \
                "STROMA_SLURM_BIN_DIR" "STROMA_SLURM_PARTITION" "STROMA_SLURM_ACCOUNT" \
                "STROMA_SLURM_SCRIPT" "STROMA_SLURM_WALLTIME" "STROMA_SLURM_CPUS" \
                "STROMA_SLURM_MEM" "STROMA_SLURM_GRES" "STROMA_MAX_BURST_WORKERS" \
                "STROMA_WARM_RESERVATION" "STROMA_LOG_DIR" "STROMA_GPU_MEM_UTIL" "STROMA_CPU_OFFLOAD_GB" \
                "STROMA_MAX_MODEL_LEN" "STROMA_MAX_NUM_SEQS" "STROMA_VLLM_CPU_KV_THREADS" \
                "STROMA_VLLM_QUANTIZATION" "STROMA_KV_CACHE_DTYPE" "STROMA_SCALE_UP_THRESHOLD" \
                "STROMA_SCALE_DOWN_IDLE_SECONDS" "STROMA_SCALE_UP_COOLDOWN" "STROMA_STATE_FILE" \
                "GATEWAY_PORT" "GATEWAY_INTERNAL_URL" "GATEWAY_VERIFY_SSL" "VLLM_BACKEND_URL" \
                "OIDC_DISCOVERY_URL" "OIDC_AUDIENCE" "JWKS_OVERRIDE_URI" \
                "KC_GATEWAY_CLIENT_ID" "KC_GATEWAY_CLIENT_SECRET" \
                "KC_OPENWEBUI_CLIENT_ID" "KC_OPENWEBUI_CLIENT_SECRET" \
                "KC_ADMIN_PASSWORD" "KC_INTERNAL_URL" "OPENWEBUI_INTERNAL_URL" "OPENWEBUI_URL")
            
            echo "" >> "${target_file}"
            echo "# Custom variables from previous config.env (preserved during merge):" >> "${target_file}"
            ...
            while IFS='=' read -r key value; do
                ...
                # Check if this variable is managed by installer
                local is_managed=0
                for managed in "${managed_vars[@]}"; do
                    if [[ "${key}" == "${managed}" ]]; then
                        is_managed=1
                        break
                    fi
...
_deploy_source_files() {
    ...
    # Skip file copy if repo and install dir are the same (e.g., /opt/stroma-ai used as shared repo)
    if [[ "$(cd "${REPO_DIR}" && pwd)" == "$(cd "${STROMA_INSTALL_DIR}" && pwd)" ]]; then
        log_ok "Repository and install directory are the same — skipping file copy."
        log_info "Adjusting permissions for in-place installation..."
