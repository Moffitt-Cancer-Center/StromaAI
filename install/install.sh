...
_deploy_config_env() {
    ...
    if [[ "${STROMA_DRY_RUN}" == "0" ]]; then
        if [[ "${_merge_config:-0}" == "1" ]]; then
            # Merge mode: preserve existing variables not managed by installer
            local temp_file="${STROMA_INSTALL_DIR}/config.env.new"
            cat > "${temp_file}" <<EOF
...
EOF
            if [[ "${_merge_config:-0}" == "1" ]]; then
                # Extract non-standard variables from old config and append to new
                ...
                echo "" >> "${temp_file}"
                echo "# Custom variables from previous config.env (preserved during merge):" >> "${temp_file}"
                ...
                mv "${temp_file}" "${STROMA_INSTALL_DIR}/config.env"
