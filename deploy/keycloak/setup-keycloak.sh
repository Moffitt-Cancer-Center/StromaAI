...
if [[ "${MODE}" == "local" ]]; then
    ...
    if [[ "${STROMA_DRY_RUN:-0}" != "1" ]]; then
        ...
        # Rootless podman with overlay force_mask makes the container filesystem
        # read-only.  Keycloak's "start" triggers a Quarkus optimized build that
        # writes into /opt/keycloak/lib/ — this fails with ReadOnlyFileSystemException.
        # "start-dev" skips the build step.  Security is unaffected: TLS termination,
        # hostname validation, and proxy headers are handled by nginx, not KC mode.
